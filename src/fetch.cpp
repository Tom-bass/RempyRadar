...
static bool              g_waterwaysFetched = false;
static int               g_waterwayRetries  = 0;
#endif
...
// ---------------------------------------------------------------------------
// Waterway fetch — Overpass API via HTTPS (reuses g_client TLS path)
// ---------------------------------------------------------------------------

#ifdef FEATURE_WATERWAYS
static bool fetchWaterways() {
    Serial.println("LOG: Waterway fetch starting");
    ...
    char bbox[64];
    snprintf(bbox, sizeof(bbox), "%.4f,%.4f,%.4f,%.4f",
             lat - dLat, lon - dLon, lat + dLat, lon + dLon);

    // Union query: rivers + coastline.
    // out geom(bbox) clips geometry to the radar bbox — without this a single coastal
    // way can have thousands of nodes.  maxsize caps the response so large-radius
    // queries fail fast instead of timing out or returning a truncated body.
    String q = "[out:json][timeout:25][maxsize:500000];"
               "(way[\"waterway\"=\"river\"](" + String(bbox) + ");"
               "way[\"natural\"=\"coastline\"](" + String(bbox) + "););"
               "out geom(" + String(bbox) + ") qt 20;";
    ...
    // Connect directly (not via httpGet) — the geometry response can be hundreds of KB,
    // which exhausts heap if accumulated in a String. Stream into deserializeJson instead.
    g_client.stop();
    g_client.setInsecure();
    ...
    Serial.printf("LOG: TLS %s to overpass-api.de | %dms | Heap: %d\n",
        connected ? "OK" : "FAIL", (int)(millis() - t0), ESP.getFreeHeap());
    if (!connected) {
        Serial.println("LOG: Waterway fetch: TLS failed");
        return false;
    }
    ...
    // Wait up to 25s for first byte
    int waited = 0;
    while (!g_client.available() && waited < 2500) {
        vTaskDelay(10 / portTICK_PERIOD_MS);
        waited++;
    }
    if (waited >= 2500) {
        Serial.println("LOG: Waterway fetch: response timeout");
        g_client.stop();
        return false;
    }
    ...
    if (!headersEnd) {
        Serial.println("LOG: Waterway fetch: no header end");
        g_client.stop();
        return false;
    }

    // Streaming geometry parser — no buffer, works for any response size.
    // Scans the g_client stream directly for "geometry":[ patterns and extracts
    // lat/lon pairs on the fly with on-the-fly chunked transfer decoding.

    deadline = millis() + 60000;

    // Detect chunked encoding by peeking at the first body byte.
    while (!g_client.available() && g_client.connected() && millis() < deadline) vTaskDelay(1);
    if (!g_client.available()) {
        Serial.println("LOG: Waterway fetch: no body");
        g_client.stop();
        return false;
    }
    bool isChunked = ((char)g_client.peek() != '{');
    int  chunkLeft = 0;
    Serial.printf("LOG: Waterway fetch: isChunked=%d\n", isChunked ? 1 : 0);

    // Read one decoded byte; handles chunked framing transparently.
    auto rb = [&](char &c) -> bool {
        while (true) {
            if (isChunked && chunkLeft == 0) {
                char t;
                do {
                    while (!g_client.available()) {
                        if (!g_client.connected() || millis() > deadline) return false;
                        vTaskDelay(1);
                    }
                    t = (char)g_client.read(); deadline = millis() + 5000;
                } while (t == '\r' || t == '\n');
                int sz = 0;
                while (true) {
                    int d = (t>='0'&&t<='9') ? t-'0' : (t>='a'&&t<='f') ? t-'a'+10 : (t>='A'&&t<='F') ? t-'A'+10 : -1;
                    if (d < 0) break;
                    sz = sz * 16 + d;
                    while (!g_client.available()) {
                        if (!g_client.connected() || millis() > deadline) return false;
            ...
            c = (char)g_client.read(); deadline = millis() + 5000;
            if (isChunked) chunkLeft--;
            return true;
        }
    };

    auto scan = [&](const char *pat) -> bool {
        int plen = (int)strlen(pat), m = 0;
        while (true) {
            char c;
            if (!rb(c)) return false;
            if (c == pat[m]) { if (++m == plen) return true; }
            else              m = (c == pat[0]) ? 1 : 0;
        }
    };

    // Returns 1="lat": found, -1=] found, 0=stream ended
    auto scanLatOrEnd = [&]() -> int {
        const char *P = "\"lat\":"; int m = 0;
        while (true) {
            char c;
            if (!rb(c)) return 0;
            if (c == ']') return -1;
            if (c == P[m]) { if (++m == 6) return 1; }
            else            m = (c == P[0]) ? 1 : 0;
        }
    };

    // Read a float; skips leading whitespace (Overpass uses "lat": -37.8 with a space).
    auto readFloat = [&](float &val) -> bool {
        char buf[24]; int len = 0; char c;
        do { if (!rb(c)) return false; } while (c == ' ' || c == '\t' || c == '\r' || c == '\n');
        if ((c>='0'&&c<='9')||c=='-'||c=='.'||c=='e'||c=='E'||c=='+') buf[len++] = c;
        else return false;
        while (len < 23) {
            if (!rb(c)) break;
            if ((c>='0'&&c<='9')||c=='-'||c=='.'||c=='e'||c=='E'||c=='+') buf[len++] = c;
    ...
    WaterPolyline polys[MAX_WATER_POLYS];
    int polyCount = 0;

    // Overpass formats geometry as `"geometry": [` (with a space before [).
    // Scan for `"geometry":` then skip any whitespace until `[` is found.
    auto scanGeomStart = [&]() -> bool {
        if (!scan("\"geometry\":")) return false;
        while (true) {
            char c;
            if (!rb(c)) return false;
            if (c == '[') return true;
    ...
    while (polyCount < MAX_WATER_POLYS) {
        if (!scanGeomStart()) break;
        Serial.printf("LOG: Waterway fetch: found geometry[%d]\n", polyCount);

        WaterPolyline &poly = polys[polyCount];
        int pts = 0, n = 0, step = 1;

        while (true) {
            if (scanLatOrEnd() != 1) break;
            float ptLat;
            if (!readFloat(ptLat)) break;
            if (!scan("\"lon\":")) break;
            float ptLon;
            if (!readFloat(ptLon)) break;

            n++;
            if ((n - 1) % step == 0) {
                poly.lats[pts] = ptLat;
                poly.lons[pts] = ptLon;
                if (++pts >= MAX_WATER_PTS_POLY) {
                    for (int i = 0; i < pts / 2; i++) {
                        poly.lats[i] = poly.lats[i * 2];
                        poly.lons[i] = poly.lons[i * 2];
                    }
                    pts /= 2;
                    step *= 2;
                }
    ...
    g_client.stop();
    displaySetWaterways(polys, polyCount);
    Serial.printf("LOG: Waterway fetch: done — %d polylines\n", polyCount);
    // Return false on 0 so fetchTask retries (up to the retry cap in fetchTask)
    return polyCount > 0;
}
#endif
